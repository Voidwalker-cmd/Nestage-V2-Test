...
import type { Stats } from "@/lib/types/staker"
...
export function StatsTable({ data }: ReferralTableProps) {
  ...
  const columns: ColumnDef<Stats>[] = [
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => {
        const levelOne = Number.parseFloat(row.getValue("levelOne"))
    ...
    },
    {
      ...
      enableSorting: false,
      cell: ({ row }) => {
        const levelTwo = Number.parseFloat(row.getValue("levelTwo"))
        const formatted = new Intl.NumberFormat("en-US", {
          style: "currency",
          currency: "USD",
        }).format(levelTwo)
